...
from dataset import generate_dataset, generate_test_loader, generate_teechdataset
...
parser.add_argument("--slice_threshold", type=float, default=0.03)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = sam_feat_seg_model_registry(num_classes=args.num_classes)
    ...
    #     if param.requires_grad and "first" in name:
    #         param.requires_grad = False
    #         print(name)
    #         # param.requires_grad =False
    #     elif param.requires_grad and "second" in name:
    #         print("冻结")
    #         print(name)
    #         param.requires_grad = False
    #     else:
    ...
    train_loader, val_loader,test_loader = generate_teechdataset(args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            torch.save(model,   "pth/"+str(epoch) +str(args.dataset) +'model.pth')
        elif epoch == 140:
            torch.save(model, "pth/" + str(epoch) + str(args.dataset) + 'model.pth')
        ...
        #     torch.save(model, 'final' + str(epoch) +str(args.dataset) +'model.pth')
    #         #_------------------------------------------------------------------------------------------------------



    torch.save(model, str(epoch) +str(args.dataset) +'model.pth')
