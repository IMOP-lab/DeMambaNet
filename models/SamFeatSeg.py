...
from models.mamba_ssm import Mamba
...
class SamFeatSeg(nn.Module):
    def __init__(
        ...
        self.first = first
        self.second = second
        self.mask_decoder_bl = seg_decoder_bl
    ...
    def forward(self,
        ...
        out = self.mask_decoder_bl(first, second )
        out = self.final1(out)
...
class SegDecoderCNN_add(nn.Module):
    def __init__(self,
                 feat_size=[48, 96, 192, 384],
                 ):
        super().__init__()
        self.feat_size = feat_size
        self.blocksintern = nn.ModuleList()
        for i in range(4):
            # s = (i + 1) // 2
            if i == 0:
                block = nn.Sequential(
                    nn.Conv2d(2*feat_size[3-i], feat_size[3-i], 3, padding=1),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(feat_size[3-i], feat_size[2-i], 3, padding=1),
                    nn.ReLU(inplace=True),
                    nn.ConvTranspose2d(feat_size[2-i], feat_size[2-i], 2, stride=2)
                )
            elif i == 3:
                block = nn.Sequential(
                    nn.Conv2d(2 * feat_size[3 - i], feat_size[3 - i], 3, padding=1),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(feat_size[3 - i], 24, 3, padding=1),
                    nn.ReLU(inplace=True),
            ...
            self.blocksintern.append(block)
        self.AFF1 = AFF(48)
        self.AFF2 = AFF(96)
        self.AFF3 = AFF(192)
        self.AFF4 = AFF(384)
        self.EVC5 = EVCBlock(768, 768)
    def forward(self, y, x):
        ...
        x =torch.cat([y, x], dim=1)  #512,64
        x = self.EVC5(x)

        for i, blk in enumerate(self.blocksintern):

            x = blk(x)    

            if i == 0:
                feature = self.AFF3(y2, x2)
                x = torch.cat([x, feature], dim=1)
                # 执行相关操作

            elif i == 1:
                feature = self.AFF2(y1, x1)
                x = torch.cat([x, feature], dim=1)
            elif i == 2:
                feature = self.AFF1(y0, x0)
                x = torch.cat([x, feature], dim=1)
        return x
    ...
    def forward(self, x):
        ...
        for level_idx, level in enumerate(self.levels):
            ...
            if level_idx in self.out_indices:
                y=x_.permute(0, 3, 1, 2).contiguous()
                seq_out.append(x_.permute(0, 3, 1, 2).contiguous())
        # output[0] [B,64,128,128]
        # output[1] [B,128,64,64]
        # output[2] [B,256,32,32]
        # output[3] [B,512,16,16]
#vit[256,64,64]
        return seq_out
